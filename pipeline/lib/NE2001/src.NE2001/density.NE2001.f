...
c	farm:	factors that multiply n_e^2 when calculating SM
	...
	real negc0, Fgc0
	   ...
	   open(11,file='/gs/project/bgf-180-ad/GBNCC/pipeline/lib/NE2001/
     &input.NE2001/gal01.inp',status='old')
	   read(11,*)
...
c read arm parameters:
        open(11, file='/gs/project/bgf-180-ad/GBNCC/pipeline/lib/NE2001/
     &input.NE2001/ne_arms_log_mod.inp', status='old')
...
c     parameter (hgc=0.026)
     ...
     .                na,ha,wa,Aa,Fa 
      ...
      if(first) then
        open(11, file='/gs/project/bgf-180-ad/GBNCC/pipeline/lib/NE2001/
     &input.NE2001/ne_gc.inp', status='old')
          read(11,*)
          read(11,*) xgc, ygc, zgc
	  read(11,*) rgc
	  read(11,*) hgc
